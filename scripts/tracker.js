...
function updateEvidence(evidenceType, initialLoad = false) {
  try {
    ...
    alert(
      "Error loading evidence, if error continues, reset evidence and try again."
    );
    console.log(e);
  }
}
...
function showPossibilities(evidence) {
  ...
  for (const ghost in ghosts) {
    if (Object.hasOwnProperty.call(ghosts, ghost)) {
      ...
      if (
        evidence[element[0]] != -1 &&
        evidence[element[1]] != -1 &&
        evidence[element[2]] != -1
      ) {
    ...
    switch (i) {
      ...
      case 2:
        tableData.appendChild(
          document.createTextNode(`Evidence ${i + 1}: ${fullItem[item]}`)
        );
        break;
      case 3:
